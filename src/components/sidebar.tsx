...
export function Sidebar({ className = "", collapsed = false, activeSection, onSectionClick }: SidebarProps) {
  ...
  return (
    <aside
      className={`flex h-full flex-col bg-[#1A1F2E] dark:bg-[#1C2127] overflow-hidden ${className}`}
    ...
    >
      ...
      <nav className={`flex-1 overflow-y-auto py-4 space-y-5 ${collapsed ? "px-2" : "px-3"}`}>
        ...
        <div>
          {!collapsed && (
            <p className="mb-1.5 px-3 text-[10px] font-semibold uppercase tracking-widest text-[#475569]">
              Main Menu
            </p>
          )}
