...
const colourExamples: { label: string; className: string }[] = [
  { label: "Default",  className: "text-slate-700" },
  { label: "Primary",  className: "text-indigo-600" },
  { label: "Success",  className: "text-emerald-600" },
  { label: "Warning",  className: "text-amber-500" },
  { label: "Danger",   className: "text-red-600" },
  { label: "Muted",    className: "text-slate-400" },
];
  ...
  return (
    <SectionWrapper id="icons" title="7. Icons">
      <div className="space-y-10">
        ...
        {/* Sizes */}
        <div>
          <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-slate-400">Sizes</p>
          <div className="flex items-end gap-8">
            {sizes.map(({ label, size }) => (
              <div key={label} className="flex flex-col items-center gap-2">
                <Icon name="bell" size={size} className="text-slate-700" />
        ...
        {/* Colours */}
        <div>
          <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-slate-400">Colours</p>
        ...
        {/* In buttons */}
        <div>
          <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-slate-400">In Buttons</p>
        ...
        <div>
          ...
          <p className="mb-4 text-xs text-slate-500">
            ...
            <code className="rounded bg-slate-100 px-1 font-mono text-[11px]">{`<Icon />`}</code>{" "}
            JSX snippet.
          </p>
          <div className="grid grid-cols-4 gap-3 sm:grid-cols-6 md:grid-cols-8">
            {allIcons.map((name) => (
              <button
                ...
                className="group flex flex-col items-center gap-2 rounded-lg border border-slate-200 p-3 transition-all hover:border-indigo-300 hover:bg-indigo-50"
              >
                <Icon
                  name={name}
                  size="md"
                ...
                />
                <span className="text-center text-[10px] leading-tight text-slate-500 group-hover:text-indigo-600">
